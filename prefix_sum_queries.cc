...
segment_tree(std::vector<T> const &x, GroupFunc) -> segment_tree<T, GroupFunc>;

auto make_prefix_sum(std::vector<ll> const &nums) {
  auto res = nums;
  for (ll i = 1; i < std::size(nums); ++i) {
    res[i] = nums[i] + res[i - 1];
  }
  return res;
}

auto make_difference(std::vector<ll> const &nums) {
  auto res = nums;
  for (ll i = 1; i < std::size(nums); ++i) {
...
auto solve(std::vector<ll> &nums,
  ...
  std::vector<node> streeNodes;
  for (auto n : nums)
    streeNodes.push_back({n, std::max(n, 0ll)});
  segment_tree stree(streeNodes, [](node a, node b) -> node {
    return {a.sum + b.sum, std::max(a.max_sum, a.sum + b.max_sum)};
  });
  for (auto [q, a, b] : queries) {
    if (q == 1) {
      ll k = a - 1;
      ll u = b;
      stree.update(k, [&](node ele) -> node {
        auto const res = ele.sum - nums[k] + u;
        return node{res, std::max(0ll, res)};
      });
      nums[k] = u;
    } else {
      --a;
      --b;
      std::cout << stree.query(a, b).max_sum << std::endl;
    }
  }
}
