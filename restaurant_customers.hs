...
solve :: [Int] -> [Int] -> Int
solve entry leave = maximum $ scanl (+) 0 $ fmap signum $ sortBy cmp $ entry ++ ((0-) <$> leave)
  where
  cmp a b = compare (abs a) (abs b)


main :: IO ()
main = do
    n <- getInt
    input <- replicateM n getInts
    let [entry, leave] = transpose input
    print $ solve entry leave
