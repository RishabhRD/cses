...
void dfs2(ll i, std::vector<std::vector<ll>> const &graph, ll kingdom,
          std::vector<ll> &kingdom_map) {
  if (kingdom_map[i] != -1)
    return;
  kingdom_map[i] = kingdom;
  for (auto n : graph[i]) {
    dfs2(n, graph, kingdom, kingdom_map);
  }
}
...
auto solve(ll n, std::vector<std::vector<ll>> const &graph,
  ...
  ll num_kingdom = 0;
  std::vector<ll> kingdom_map(n, -1);
  {
    for (ll n : order) {
      if (kingdom_map[n] == -1) {
        ++num_kingdom;
        dfs2(n, op_graph, num_kingdom, kingdom_map);
      }
    }
  }
  std::cout << num_kingdom << std::endl;
  for (auto n : kingdom_map)
    std::cout << n << ' ';
  std::cout << std::endl;
}
