...
auto solve(std::vector<ll> const &nums,
  ...
  std::vector diff = nums;
  for (ll i = 1; i < std::size(nums); ++i) {
    diff[i] = nums[i] - nums[i - 1];
  }
  segment_tree stree(diff, std::plus<>{});
  for (auto const &query : queries) {
    auto const q = query[0];
    if (q == 1) {
      auto const a = query[1] - 1;
      auto const b = query[2] - 1;
      auto const k = query[3];
      stree.update(a, [k](auto ele) { return ele + k; });
      if (b != std::size(diff) - 1)
        stree.update(b + 1, [k](auto ele) { return ele - k; });
    } else {
      auto const k = query[1] - 1;
      std::cout << stree.query(0, k) << std::endl;
    }
  }
}
...
int main() {
  ...
  for (ll i = 0; i < q; ++i) {
    ...
    if (q == 1) {
      ...
      auto const u = read<ll>();
      queries.push_back(std::vector({q, a, b, u}));
    } else {
      auto const k = read<ll>();
      queries.push_back(std::vector({q, k}));
    }
  }
  solve(nums, queries);
}
