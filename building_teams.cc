...
bool dfs(graph_t const &graph, ll i, int prev, std::vector<int> &color) {
  if (color[i] != 0) {
    return color[i] != prev;
  }
  auto const cur = prev == 1 ? 2 : 1;
  color[i] = cur;
  return std::all_of(std::begin(graph[i]), std::end(graph[i]),
                     [&](auto const n) { return dfs(graph, n, cur, color); });
}

auto solve(std::vector<std::pair<ll, ll>> const &edges, ll n) {
  std::vector color(n + 1, 0);
  auto const graph = build_graph(edges, n);
  auto const is_possible = [&] {
    for (ll i = 1; i <= n; ++i) {
      if (color[i] == 0)
