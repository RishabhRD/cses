...
auto solve(std::vector<ll> const &rooms, std::vector<ll> const &queries) {
  segment_tree stree{rooms, [](auto a, auto b) { return std::max(a, b); }};
  for (auto q : queries) {
    auto const predicate = [&](auto k) { return stree.query(0, k) < q; };
    auto idx = binary_search(0, std::size(rooms), predicate);
    if (idx == std::size(rooms)) {
      std::cout << 0 << std::endl;
    } else {
      std::cout << idx + 1 << std::endl;
      stree.update(idx, [&](auto ele) { return ele - q; });
    }
  }
}
...
int main() {
  ...
  auto const h = read_vec<ll>(n);
  auto const q = read_vec<ll>(m);
  solve(h, q);
}
