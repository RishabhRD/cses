...
auto solve(std::vector<ll> const &salaries,
           std::vector<std::tuple<char, ll, ll>> &queries) {
  auto const get_bucket = [](auto salary) { return salary / 100; };
  std::map<ll, ll> freq;
  std::vector<ll> init_bucket(1e7);
  for (auto salary : salaries) {
    ++freq[salary];
    ++init_bucket[get_bucket(salary)];
  }
  segment_tree stree(init_bucket, std::plus<>{});
  for (auto [q, a, b] : queries) {
    if (q == '?') {
      auto const ba = get_bucket(a);
      auto const bb = get_bucket(b);
      auto ans = stree.query(ba, bb);
      {
        auto i = ba * 100;
        for (; i < a; ++i) {
          ans -= freq[i];
      ...
      auto const a_salary = salaries[a - 1];
      auto const a_bucket = get_bucket(a_salary);
      auto const b_bucket = get_bucket(b);
      stree.update(a_bucket, [](auto ele) { return ele - 1; });
      stree.update(b_bucket, [](auto ele) { return ele + 1; });
      --freq[a_salary];
      ++freq[b];
    }
  }
...
int main() {
  ...
  auto const salaries = read_vec<ll>(n);
  std::vector<std::tuple<char, ll, ll>> queries(q);
  for (auto &[q, a, b] : queries) {
    std::cin >> q >> a >> b;
  }
  solve(salaries, queries);
}
