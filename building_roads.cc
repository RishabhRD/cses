...
auto solve(std::vector<std::pair<ll, ll>> const &roads, ll n) {
  dsu dsu(n + 1);
  for (auto [x, y] : roads)
    dsu.combine(x, y);
  ...
  std::vector<ll> to_connect{st.begin(), st.end()};
  std::cout << to_connect.size() - 1 << std::endl;
  for (ll i = 0; i < std::size(st) - 1; ++i) {
    std::cout << to_connect[i] << ' ' << to_connect[i + 1] << std::endl;
  }
}
...
int main() {
  ...
  std::vector<std::pair<ll, ll>> roads;
  for (ll i = 0; i < m; ++i) {
