...
ll dfs(ll i, ll parent, std::vector<std::vector<ll>> const &tree,
  ...
  idx[i] = dfs_tree.size();
  dfs_tree.push_back(vals[i]);
  subtree_size[i] = 1;
  for (auto n : tree[i]) {
    if (n != parent) {
      subtree_size[i] += dfs(n, i, tree, vals, subtree_size, idx, dfs_tree);
    }
  }
  return subtree_size[i];
}

auto solve(std::vector<ll> const &vals,
           std::vector<std::pair<ll, ll>> const &edges,
           std::vector<std::vector<ll>> const &queries) {
  auto const n = std::size(vals) - 1;
  auto const tree = build_tree(n, edges);
  std::vector<ll> subtree_size(n + 1);
  std::vector<ll> idx(n + 1);
  std::vector<ll> dfs_tree;
  dfs(1, 1, tree, vals, subtree_size, idx, dfs_tree);
  segment_tree stree(dfs_tree, std::plus<>{});
  for (auto const &query : queries) {
    if (query[0] == 1) {
      auto const s = query[1];
      auto const x = query[2];
      stree.update(idx[s], [x](auto _) { return x; });
    } else {
      auto const s = query[1];
      std::cout << stree.query(idx[s], idx[s] + subtree_size[s] - 1)
                << std::endl;
    }
  }
}
