...
ll dfs(ll i, ll prev, std::vector<std::vector<ll>> const &tree,
       std::vector<ll> &dp) {
  if (dp[i] != -1)
    return dp[i];
  ll total_sum = 0;
  ...
  for (auto n : tree[i]) {
    if (n != prev) {
      ...
      cur_sum -= dfs(n, i, tree, dp);
      for (auto nn : tree[n]) {
        if (nn != i) {
          cur_sum += dfs(nn, n, tree, dp);
        }
      }
      max_sum = std::max(max_sum, 1 + cur_sum);
    }
  }
  return dp[i] = max_sum;
}
...
auto solve(ll n, std::vector<std::pair<ll, ll>> const &edges) {
  ...
  std::cout << dfs(0, -1, tree, dp) << std::endl;
}
